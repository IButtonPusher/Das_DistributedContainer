...
public partial class BaseResolver
{
   ...
   {
      return await ResolveAsyncImpl<T>(_emptyCtorParams, GetDefaultCancellationToken(), true)
                                                           ...
                                                           Boolean isWaitIfNotFound)
   {
      var typeO = await _typeMappings.GetMappingAsync(contractType, cancellationToken,
         isWaitIfNotFound).ConfigureAwait(false);
      ...
      var waiter = await _instanceMappings.TryGetWaiterAsync(typeO)
                                          .ConfigureAwait(false);
      if (waiter != null)
      ...
      {
         found = await bldr.GetAwaiter().ConfigureAwait(false);
         return found;
      }
                                             ...
                                             Boolean isWaitIfNotFound)
   {
      ...
      var res = await PerformObjectResolutionAsync(typeI, ctorArgs,
         cancellationToken, isWaitIfNotFound).ConfigureAwait(false);
